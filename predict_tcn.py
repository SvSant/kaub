...
def windowed_data(data, window_size, pred_size):
    ...
    X = np.zeros([dim[0]-window_size-pred_size, window_size, dim[1]])
...
def create_model():
    ...
    inputs = keras.Input(shape=(G.WINDOW_SIZE, 19))
    x = TCN(nb_filters=G.FILTERS,kernel_size=G.KERNEL_SIZE,dilations=(1,2,4,8))(inputs)
...
def test_step(X,y_true):
    ...
    # loss_value = tf.compat.v1.losses.huber_loss(y_true, y_pred)
    loss_value = tf.keras.metrics.mean_squared_error(y_true, y_pred)
    # loss_value = tf.reduce_sum(tf.abs(y_pred - y_true))
...
save_title = '_tcn_'+str(G.EPOCHS)+'_'+str(G.LAYERS)+'_'+str(G.FILTERS)+'_dr_'+str(G.DROPOUT)+'_lr_'+str(G.LEARNING_RATE)+'_pred_'+str(G.PRED_SIZE)+'_win_'+str(G.WINDOW_SIZE)
...
for epoch in range(1,G.EPOCHS+1):
    ...
    np.savetxt('tcn/loss_history/train_loss_'+save_title+'.csv', train_loss_plot)
    np.savetxt('tcn/loss_history/test_loss_'+save_title+'.csv', test_loss_plot)
...
# make plot title
plot_title = 'Epochs = '+str(G.EPOCHS)+', pred_days = '+str(G.PRED_SIZE)+'\n Layers = '+str(G.LAYERS)+', Width = '+str(G.FILTERS)+', Learning_rate = '+str(G.LEARNING_RATE)+', Dropout = '+str(G.DROPOUT)+'\n MSE = '+str(mse_all)+', MAE = '+str(mae_all)

if not os.path.exists('tcn/predictions/'+save_title):
   os.makedirs('tcn/predictions/'+save_title)
...
plt.savefig('tcn/loss_history/loss'+save_title+'.png')
plt.close()
...
for i in range(G.PRED_SIZE):
    ...
    plt.savefig('tcn/predictions/'+save_title+'/test_day_'+str(i+1)+'.png')
    plt.close()
...
plt.savefig('tcn/predictions/'+save_title+'/test_error.png')
plt.close()
